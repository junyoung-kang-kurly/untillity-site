...
(function (global) {
   ...
   * 글자 뒤 주머니(scrim).
   *
   * ★ **화면이 아니라 글자 컬럼에 맞춘 타원이다.** 앱은 `scrim.frame` 을
   * `focus.insetBy(dx: -w*0.45, dy: -h*0.22)` 로 잡는다 — 컬럼 폭의 1.9배, 높이의 1.44배.
   * 화면 대각선 기준의 원으로 깔면 가운데가 통째로 어두운 돔이 된다 (2026-09-18 실측:
   * 밝은 페이블에서 특히 티가 난다).
   */
  function bakeScrim(g, w, h, theme, p, strength) {
    const c = theme === 'fable' ? mix(p.base[1], [0, 0, 0], 0.5)
            : theme === 'astra' ? mix(p.base[0], [0, 0, 0], 0.7)
            : mix(p.base[0], [0, 0, 0], 0.6);
    const a = (theme === 'fable' ? 0.50 : theme === 'astra' ? 0.42 : 0.60) * (strength === undefined ? 1 : strength);
    if (a <= 0.001) return;
    const rx = (w * 0.46 * 1.9) / 2;      // 컬럼 46% (SPEC F14.9) × 1.9
    const ry = (h * 0.40 * 1.44) / 2;
    g.save();
    g.translate(w / 2, h / 2);
    g.scale(rx / ry, 1);
    const gr = g.createRadialGradient(0, 0, 0, 0, 0, ry);
    gr.addColorStop(0, rgb(c, a));
    gr.addColorStop(0.5, rgb(c, a * 0.72));
  ...
  function createBackdrop(canvas, opts) {
    ...
    // 앱 화면을 그대로 옮길 때는 1. **페이지 배경으로 깔 때는 낮춘다** — 글자 컬럼이 늘 가운데 있지 않다.
    const scrimStrength = opts.scrimStrength === undefined ? 1 : opts.scrimStrength;
    ...
    function bake() {
      ...
      if (scrim) bakeScrim(g, bw, bh, theme, p, scrimStrength);
      // 비네트
